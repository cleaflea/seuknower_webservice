...
def parserHtml(request, cardNumber, academicYear):
    ...
    if match:
        ...
        # 因为可能会出现只有上课是哪一节而没有上课周次的情况，但是一般情况下用下面的代码处理过之后每一节课列表里第一个元素是课程名称，第二个元素是上课周次，第三个元素是上课是哪一节到哪一节，如果没有周次的话为了保证格式的统一就插入一个空格，所以else后面insert的东西可以任何字符串，主要是为了保证和普通情况解析出来的数据的格式保持一致
